...
namespace loki::position
{
	...
	search_position_t make(game_state_t state, magics::magic_index_t bishop_index, magics::magic_index_t rook_index)
	{
		auto ptr = new search_position(
			...
			std::make_unique<move_generator>(rook_index, bishop_index));
		return search_position_t(ptr);
	}

	search_position::search_position(std::unique_ptr<game_state> state, std::unique_ptr<move_generator> move_generator)
		: m_state{ std::move(state) }, m_move_generator{ std::move(move_generator) }, m_history{}
	{
		// Fill m_piecebbs
